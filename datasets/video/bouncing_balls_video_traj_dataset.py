...
class BouncingBallsTrajDataset(BaseVideoDataset):
    '''
    ...
    def get_data_lengths(self, split):
        if split == 'training':
            lengths = [48] * len(self.get_data_paths(split))
        else:
            lengths = [288] * len(self.get_data_paths(split))
        return lengths
    ...
    def __getitem__(self, idx):
        ...
        video = video['positions'][frame_idx : frame_idx + self.n_frames, :, :-1].astype(np.float32) # T x 2 x 4 (last_dim = x,y,vx,vy)
        ...
        video = video.float().view(video.size(0), -1)\
